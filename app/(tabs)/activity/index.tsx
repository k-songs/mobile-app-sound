...
export default function Index() {
  ...
  const orientationLockRef = useRef<boolean>(false); // 방향 잠금 상태 추적
  ...
  // 화면 크기 변화를 감지해서 가로 모드 여부 판단
  useEffect(() => {
    const isLandscape = width > height;
    console.log(`📐 Screen dimensions: ${width}x${height}, isLandscape: ${isLandscape}`);
    
    if (isLandscape && !orientationLockRef.current) {
      // 가로 모드이고 아직 잠금이 안 되어 있으면 준비 완료
      console.log('✅ Landscape detected - ready to show');
      setIsOrientationReady(true);
      orientationLockRef.current = true;
    } else if (!isLandscape && orientationLockRef.current) {
      // 세로 모드로 변했으면 다시 가로 모드로 전환 필요
      console.log('🔄 Portrait detected - need to lock landscape');
      setIsOrientationReady(false);
      orientationLockRef.current = false;
    }
  }, [width, height]);

  // 컴포넌트 마운트 시 가로 모드로 잠금
  useEffect(() => {
    let isMounted = true;
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('🎯 Tab focused - checking orientation');
      
      const recheckOrientation = async () => {
        try {
          // 현재 방향 확인
          const currentOrientation = await ScreenOrientation.getOrientationAsync();
          console.log('Current orientation on focus:', currentOrientation);
          ...
          if (currentOrientation === ScreenOrientation.Orientation.PORTRAIT_UP || 
            ...
            console.log('🔄 Portrait detected on focus - locking to landscape');
            await ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE);
          }
        } catch (error) {
          console.error('Failed to recheck orientation on focus:', error);
        }
      };
      
      // 약간의 딜레이 후 확인
      const timer = setTimeout(recheckOrientation, 100);
  ...
  const handleExit = async () => {
    try {
      ...
      console.log('✅ Restored to portrait mode');
  ...
  // 화면 방향 전환 중이면 로딩 화면 표시
  if (!isOrientationReady) {
    return (
      <SafeAreaView style={[styles.fullScreen, { justifyContent: 'center', alignItems: 'center', backgroundColor: '#1a1a1a' }]}>
        {/* 나가기 버튼은 로딩 중에도 표시 */}
        <TouchableOpacity 
